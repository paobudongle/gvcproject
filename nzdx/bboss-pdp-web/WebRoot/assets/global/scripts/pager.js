var bboss = function () {};
bboss.pager =  function(options){
	
	var $pagerevent = options ?options.pagerevent:{};
	if(!$pagerevent )$pagerevent={};
	return {
		/**
...
//		},
		...
		//	}
		//}
		__handleContainerid:function(containerid){
//			if(containerid){
...
//			}
		...
		loadPageContent:function (pageurl, containerid, selector) {
			containerid = this.convertValue(containerid, false);	
			...
			if(flag === false)
				return;
			_containerid = this.__handleContainerid(containerid);
			if (selector && selector != "") {
				
				_selector =  this.__handleContainerid(selector);
				$(_containerid).load(pageurl + " " + _selector,function(){
					//setTable_grayCss();
					if($.parser)	$.parser.parse(_containerid);
					try{
				...
				$(_containerid).load(pageurl,function(){
					try{
						setTable_grayCss()
					}catch(e){
