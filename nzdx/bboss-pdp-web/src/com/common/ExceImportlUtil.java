...
public  class ExceImportlUtil {
    ...
    /**
     * 描述：获取IO流中的数据，组装成List<List<Object>>对象自定义读取sheet,row,cell
     * @param fileName
     * @return
     * @throws IOException
     */
    public  List<List<Object>> getBankListByExcel(MultipartFile file, int sheetNum, int startRow) throws Exception{
        ...
        for (int j = startRow; j <= sheet.getLastRowNum(); j++) {
            ...
            List<Object> li = new ArrayList<Object>();
            for (int y = row.getFirstCellNum(); y < row.getLastCellNum(); y++) {
                System.out.println(sheetNum+"，"+j+","+y);
                cell = row.getCell(y);
                if(cell == null) {
                    continue;
                }
